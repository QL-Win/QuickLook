...
namespace QuickLook.Plugin.ImageViewer.Webview.Drawio;

public class DrawioImagePanel : SvgImagePanel
{
    private string _drawioContent;

    public DrawioImagePanel()
    {
#if false // The current draw.io rendering is not suitable for force dark mode
    ...
    public override void Preview(string path)
    {
        ...
        _drawioContent = File.ReadAllText(path);
        _homePage = _resources["/drawio2html.html"];
    ...
    {
        if (!e.IsSuccess || _drawioContent == null)
            return;

        var jsonString = EscapeJsonString(_drawioContent);
        var script = $"(function(){{var x={jsonString};function r(){{if(typeof window.__DRAWIO_RENDER==='function'){{window.__DRAWIO_RENDER(x);}}else{{setTimeout(r,20);}}}}r();}})();";

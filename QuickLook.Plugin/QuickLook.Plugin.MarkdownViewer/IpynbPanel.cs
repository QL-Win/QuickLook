...
public class IpynbPanel : WebpagePanel
{
    ...
    private byte[] _ipynbBytes;
    private string _ipynbName;
    ...
    public void PreviewIpynb(string path)
    {
        FallbackPath = Path.GetDirectoryName(path);
        _ipynbBytes = File.ReadAllBytes(path);
        _ipynbName = Path.GetFileName(path);
        _homePage = Encoding.UTF8.GetBytes(MarkdownPanel.ReadString("/ipynb2html.html"));
    ...
    protected override void WebView_NavigationCompleted(object sender, CoreWebView2NavigationCompletedEventArgs e)
    {
        ...
        if (!e.IsSuccess || _ipynbBytes == null || _webView?.CoreWebView2 == null)
            return;

        var base64 = Convert.ToBase64String(_ipynbBytes);
        var nameJson = EscapeJsonString(_ipynbName ?? "file.ipynb");
        var json = $"{{\"type\":\"open-ipynb\",\"payload\":{{\"base64\":\"{base64}\",\"name\":{nameJson}}}}}";
    ...
    protected override void WebView_WebResourceRequested(object sender, CoreWebView2WebResourceRequestedEventArgs args)
    {
        ...
        try
        {
            ...
            if (Path.IsPathRooted(potentialAbsolutePath) && File.Exists(potentialAbsolutePath))
                ...
                // Treat as relative path (e.g. markdown cell images)

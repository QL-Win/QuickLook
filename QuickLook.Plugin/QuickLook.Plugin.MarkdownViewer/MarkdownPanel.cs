...
public class MarkdownPanel : WebpagePanel
{
    ...
    protected virtual string PrepareMarkdownContent(string path, string content)
    {
        var extension = Path.GetExtension(path);

        // .mmd is ambiguous: MultiMarkdown or Mermaid diagram source
        if (extension.Equals(".mmd", StringComparison.OrdinalIgnoreCase)
            && MermaidPanel.IsLikelyMermaidDocument(content))
            return MermaidPanel.WrapAsMermaidCodeFence(content);
